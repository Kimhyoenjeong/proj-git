	...
	<nav class="navbar navbar-default">
		<div class="navbar-header">
			...
			<a class="navbar-brand" href="main.jsp">
				<p style="font-weight: bold">모두의 레시피</p>
			</a>
		</div>
			...
			<table class="table table-haver" style="border: 1px solid #dddddd">
					...
					<tr>
						...
						<%	JjimDAO jjimDAO = new JjimDAO();
							...
							if (list1.isEmpty()){%>
						<td align="right" bgcolor="beige"><button onclick = "location.href='jjimAction.jsp?bbsID=<%=bbsID%>'" class="btn btn-secondary">찜하기</button></td>
						<%	}
							else{%>
						<td align="right" bgcolor="beige"><button onclick = "location.href='jjimAction.jsp?bbsID=<%=bbsID%>'" class="btn btn-warning">찜해제</button></td>
						<%	} %>						
					</tr>
					<%} 
					if (boardID == 2){%>
				...
				//만약 프로그램을 다른 컴퓨터로 옮길 경우 경로를 바꿔주어야함
					String real = "C:\\Users\\Me\\Desktop\\h\\proj\\.metadata\\.plugins\\org.eclipse.wst.server.core\\tmp1\\wtpwebapps\\morae\\bbsUpload";
				...
				%>
					...
					<td colspan="6" align="center">
						<button onclick="location.href='evaluationAction.jsp?likeEat=1&sosoEat=0&badEat=0&bbsID=<%=bbsID%>'" class="btn btn-secondary">좋아요 <%=like %></button>	
						<button onclick="location.href='evaluationAction.jsp?likeEat=0&sosoEat=1&badEat=0&bbsID=<%=bbsID%>'" class="btn btn-secondary">그럭저럭 <%=soso %></button>	
						<button onclick="location.href='evaluationAction.jsp?likeEat=0&sosoEat=0&badEat=1&bbsID=<%=bbsID%>'" class="btn btn-secondary">맛없어요 <%=bad %></button>	
					</td>
					</tr>
					<tr>
					<%
					...
					<a href="bbs.jsp?boardID=<%=boardID %>" class="btn-warning" >목록</a>
					<%
						if(userID != null && userID.equals(bbs.getUserID())){
					%>
							<a href = "update.jsp?bbsID=<%= bbsID %>&boardID=<%=boardID %>" class="btn-warning">수정</a>
							<a onclick="return confirm('정말로 삭제하시겠습니까?')" href = "deleteAction.jsp?bbsID=<%= bbsID %>&boardID=<%=boardID %>" class="btn-warning">삭제</a>
					</td>
					</tr>
					<% } %>
			</table>		
								...
								<div class="row">
									<table class="table table-striped" style="text-align: center; border: 1px solid #dddddd">
										...
										<td align="right"><%
												...
												%>
														<form name = "p_search">
															<a type="button" onclick="nwindow(<%=boardID%>,<%=bbsID %>,<%=list.get(i).getCommentID()%>)" class="btn-warning">수정</a>
														</form>	
														<a onclick="return confirm('정말로 삭제하시겠습니까?')" href = "commentDeleteAction.jsp?bbsID=<%=bbsID %>&commentID=<%= list.get(i).getCommentID() %>" class="btn-warning">삭제</a>
										...
										<tr>
											...
											<% 	
												String commentReal = "C:\\Users\\Me\\Desktop\\h\\proj\\.metadata\\.plugins\\org.eclipse.wst.server.core\\tmp1\\wtpwebapps\\morae\\commentUpload";
			...
			<form method="post" encType = "multipart/form-data" action="commentAction.jsp?bbsID=<%= bbsID %>&boardID=<%=boardID%>">
					<table class="table table-striped" style="text-align: center; border: 1px solid #dddddd">
						<tr>
							...
							<td><br><br><input type="submit" class="btn btn-warning pull" value="댓글 작성"></td>
						</tr>
						<tr>
