 	...
 	vw.ol3.vworldCategory = {
 	 ...
 	 vw.ol3.ImgUrl  = {
    		oltheme: vw.ol3.BaseURLs.vworldUrl + "/images/maps/",
   		    ...
   		    panzoomsliderthumbtipbox: vw.ol3.BaseURLs.vworldUrl + "/images/maps/zoom_box.png" ,
  ...
  _noticeControl = new ol.control.Control(
   ...
   return _noticeControl;
	...
	vw.ol3.Layers.prototype._init = function() {
		...
		var maxExtent = ol.proj.transformExtent([117.83,45.09,135.07,31.92], 'EPSG:4326', 'EPSG:900913');
		this.groupLayer.get(vw.ol3.BasemapType.GRAPHIC).setExtent(maxExtent);
		this.groupLayer.get(vw.ol3.BasemapType.GRAPHIC_GRAY).setExtent(maxExtent);
		this.groupLayer.get(vw.ol3.BasemapType.GRAPHIC_NIGHT).setExtent(maxExtent);
		this.groupLayer.get(vw.ol3.BasemapType.PHOTO).setExtent(maxExtent);
		this.groupLayer.get("hybrid").setExtent(maxExtent);
	...
	vw.ol3.Layers.prototype._setLayer = function(basemapType) {
		...
		this._setVisible(osmLayer, true);	   
   		switch(basemapType) {
	   		case vw.ol3.BasemapType.PHOTO_HYBRID :   			
	   			this._addLayer(this.groupLayer.get(vw.ol3.BasemapType.PHOTO));
	   			this._addLayer(this.groupLayer.get("hybrid"));
	   			this.groupLayer.get(vw.ol3.BasemapType.PHOTO).setZIndex(1);
	   			this.groupLayer.get("hybrid").setZIndex(2);
		...
		if ( this.sliderVisible ) {			
			...
			_objOpt.className = "ol-zoomin";
  			_objOpt.alt = "확대";
   			...
   			vw.ol3.CommonFunc._remove(".ol-zoomslider");
   			vw.ol3.CommonFunc._remove(".ol-zoomslider-thumb");
	   		this._zoomslider = new ol.control.ZoomSlider(_option);   		
	   		_thisVmap.addControl(this._zoomslider);
   			...
   			_toolTipBox = vw.ol3.CommonFunc._createElement("div");
	   		_toolTipBox.style.width		= "32px";
		   	_toolTipBox.style.height	= "18px";
		   	_toolTipBox.style.position = "relative";
		   	_toolTipBox.style.top = "146px";   			
		   	_toolTipBox.style.left = "56px";
		   	_toolTipBox.style.paddingLeft = "13px";
   			$(_toolTipBox).html(_thisVmap.getView().getZoom());
	   		var toolTipBoxFunc = function() {
	   			setTimeout( 
	   					function () { 	
	   						var curZoom = _thisVmap.getView().getZoom();
	   			   			_toolTipBox.style.width		= "32px";
	   			   			_toolTipBox.style.height	= "18px";
	   			   			_toolTipBox.style.position = "relative";
	   			   			_toolTipBox.style.top = "146px";   			
	   			   			_toolTipBox.style.left = "56px";
	   			   			_toolTipBox.style.paddingLeft = "13px";
	   			   			_toolTipBox.style.backgroundImage = "url('" + vw.ol3.ImgUrl.panzoomsliderthumbtipbox + "')";	   						
	   			   			// 디폴트 7을 기준으로 7보다 높으면 top에서 7만큼 제외, 7보다 낮으면 7만큼 추가.	   			
	   			   			var _top = $(_toolTipBox).css("top");
	   			   			var _sliderTop = $(".ol-zoomslider-thumb").css("top");
	   		   	   			_top = Number(_top.substring(0,_top.length-2));
	   		   	   			_sliderTop = Number(_sliderTop.substring(0,_sliderTop.length-2));
	   		   	   			_top = _sliderTop + 63; 
	   		   	   		   	$(_toolTipBox).html(curZoom);
	   		   	   		   	$(_toolTipBox).css("top", _top + "px");
	   					} , 
	   			1);
	   		};
	   		vw.ol3.CommonFunc._append(this.divElement, ".ol-zoomslider");
	   		vw.ol3.CommonFunc._append(this.divElement, _toolTipBox);
	   		_thisVmap.on("moveend", toolTipBoxFunc);
		} else {
			// 슬라이더뺌.
			_height = "96px";			
			_objOpt.className = "ol-zoomin";
