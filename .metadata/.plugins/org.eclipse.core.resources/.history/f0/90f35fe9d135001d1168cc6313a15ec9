...
//타이틀 추가
Raphael.fn.title = function (a, d, b) {
    var c = this.set();
    b = this.text(a, d, b).attr(Raphael.g.titleattr);
    return c.push(b.title(), b)
};
//서브타이틀 추가
Raphael.fn.subtitle = function (a, f, d, b, c) {
    var e = this.set();
    var dset = this.set();
    //서브타이틀 분리
    /*var dlist = d.split('\n');
    ...
    if (n > 1) {
	    ...
	    for (var i = 1; i < n; i ++){
	    	y = f + 12*i;
	    	var d2 = this.text(a, y, dlist[i]).attr(Raphael.g.txtattr);
	    	dset.push(d2);
	    }
	    var d1 = this.text(a, f, dlist[0]).attr(Raphael.g.subtitleattr);
	    dset.push(d1);	    
	    return e.push(dset.popup(b, c), dset);
    } else {
    	d = this.text(a, f, d).attr(Raphael.g.subtitleattr);
...
Raphael.fn.popup = function (a, f, d, b, c) {
    var e = this.set();
    d = this.text(a, f, d).attr(Raphael.g.txtattr);//modify   labelattr
    return e.push(d.popup(b, c), d)
};
...
(function () {
    var c = ["lighter", "darker", "resetBrightness"],
        ...
        a = ["popup", "tag", "flag", "label", "drop", "blob", "title"];//title 추가
    for (var b in a) {
        (function (d) {
...
Raphael.g = {
    ...
    //타이틀 추가
    titleattr: {
        font: "13px Arial, sans-serif",fill: "#000","font-weight":"400","text-anchor": "middle"
    },
    //팝업추가
    subtitleattr: {
        font: "12px Arial, sans-serif",fill: "#fff","font-weight":"400"
    },    
    labelattr: {
        font: "12px Arial, sans-serif",
        fill: "#fff"
        ,"text-anchor": "middle" //modify
    },
    colors: (function () {
    ...
    snapMax: function(j){
    	...
    	var len = j.length;
    	for (var b = 0; b < len; b++) {
    		max = Math.max(j[b],max);
        }
        return max
    },
    snapMinMax: function(j){
    	var min = j[0];
    	var max = j[0];
    	var len = j.length;
    	for (var b = 0; b < len; b++) {
    ...
    axis_nox: function (p, o, k, D, e, G, g, J, h, a, q) {
    	a = a == null ? 2 : a;
        h = h || "t";
        G = G || 10;
        q = arguments[arguments.length - 1];
